...
\title{The \textsf{luaaccents} package\\[1ex]
...
\begin{description}
  \item[\texttt{\textbackslash LuaAccentsOff}] stop rewriting input
    lines (takes effect from the next input line);
  \item[\texttt{\textbackslash LuaAccentsOn}] resume rewriting.
\end{description}
...
\begin{itemize}
  \item The rewriting operates on \emph{every} input line, in text
    ...
    \verb|\LuaAccentsOff| around text where combining characters must
    be left alone.
